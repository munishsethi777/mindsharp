...
public class UserDataStore implements UserDataStoreI, RowMapper {
	...
	private final static String SELECT_BY_SEQ = SELECT_COMMON
			+ " where users.seq = ?";
	private final static String SELECT_UNAME = SELECT_COMMON
			+ " where users.username = ?";
	private final static String SELECT_UNAME_PASS = SELECT_COMMON
	...
	public User findBySeq(long seq) {
		...
		return (User) persistenceMgr.executeSingleObjectQuery(SELECT_BY_SEQ,
				params, this);
	}
	...
	public User findByUserName(String userName) {
		...
		return (User) persistenceMgr.executeSingleObjectQuery(SELECT_UNAME,
				params, this);
	}
	...
	public User findByUserNamePassword(String userName, String password) {
		...
		User user = (User) persistenceMgr.executeSingleObjectQuery(
				SELECT_UNAME_PASS, params, this);
		return user;
	}
				...
				try {
					...
					for (String str : mySkillsArr) {
						try {
							...
							logger.error(
									"Error converting to GameSkillType from "
											+ str, e);
						}
					}
					user.setMySkills(mySkills);
					UserBadgeDataStoreI UBDS = ApplicationContext
							.getApplicationContext().getDataStoreMgr()
							.getUserBadgeDataStore();
					List<GameSkillBadgeType> mySkillBadges = UBDS
							.findBadges(userSeq);
	...
	private void saveFileContent(User user) throws Exception {
		String fullImageFileName = "D:\\projects\\Java Projects\\mindSharp\\images\\userImages\\"
				+ user.getImageName();
		...
		String newImageName = String.valueOf(user.getSeq()) + "_thumb" + "."
				+ ImageUtils.getImageExtension(user.getImageName());
