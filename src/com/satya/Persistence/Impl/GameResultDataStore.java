...
public class GameResultDataStore implements GameResultDataStoreI,RowMapper {
	...
	public int findMaxScore(long gameSeq, long userSeq) {
		...
		return (Integer) persistenceMgr.executeSingleObjectQuery(FINDMAXSCORE, params, new RowMapper() {
	...
	public void SaveGameResult(GameResult obj) {
		try{
			Object[] params = new Object[]{
					...
					obj.getCorrect(),
					obj.getLevels(),
					obj.getTotalAttempts(),
					obj.getTimePlayedSeconds(),
					obj.getOpportunities(),
					obj.getAccuratePercent(),
					obj.getReactionTimeSeconds()};
	...
	public List<GameResult> findResultsByUser(long userId) {
		...
		return (List<GameResult>) persistenceMgr.executePSQuery(FINDRESULTSBYUSER, params, this);
	}
	...
	protected Object populateObjectFromResultSet(ResultSet rs)throws SQLException{
		...
		try{
			...
			gameResult.getGame().setGameSkillType(GameSkillType.valueOf(skillType));
		}catch(Exception e){}
